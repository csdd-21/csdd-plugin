...
app.get('/tags', function (req, res) {
    models.Plugin.aggregate([{ $match: { publish: 1 } }, { $group: { _id: '$tag' } }, { $sort: { _id: 1 } }], function (
        err,
        datas
    ) {
        if (!err) {
...
app.post('/plugins', function (req, res) {
    console.log('plugins req ------', req.body)
    let params = {}
    ...
    Promise.all([promise1, promise2])
        .then((datas) => {
            ...
            console.log('plugins res ---', { total: datas[0], datas: resDatas })
            res.json({ total: datas[0], datas: resDatas })
        })
        .catch((err) => {
...
app.post('/submit', upload.single('file'), function (req, res, next) {
    ...
    fs.renameSync(path.join(process.cwd(), "/" + req.body.imgPath), path.join(process.cwd(), targetPath), function (err) {
        if (err) {
    ...
    models.Plugin(params).save((err, data) => {
        console.log('submit params ---', params, 'err --', err, 'data --', data)
        if (err) {
