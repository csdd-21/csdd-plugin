...
app.post("/submit", upload.single('file'), function (req, res, next) {
    ...
    // 提交图片
    let targetPath = '/imgs/' + req.body.title + '.' + req.body.icon.split('\\')[1].split('.')[1]
    fs.rename(path.join(process.cwd(), "/" + req.body.icon), path.join(process.cwd(), '../../public' + targetPath), function (err) {
        if (err) {
