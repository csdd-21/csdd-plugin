...
app.post("/upload",upload.single('file'),function(req,res,next) {
	...
	fs.rename(path.join(process.cwd(),"/" + req.file.path),     path.join(process.cwd(),'../src/assets/test'+"." + ext),     function(err,doc){
		if(err) {
			return res.sendResult(null,400,"上传文件失败");
		}

        console.log('process.cwd() -----------',process.cwd());
        console.log('path.join(process.cwd(),targetPath) -----------',path.join(process.cwd(),targetPath));
