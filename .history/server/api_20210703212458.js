...
// app.post("/submitPng", upload.single('file'),  async function (req, res, next) {})
app.get("/submit", function (req, res, next) {
    ...
    // console.log('00000000000', path.join(process.cwd(), "/" + req.body.imgPath));
    // console.log('111111111', path.join(process.cwd(), '../public/' + targetPath));



    // let readStream = fs.createReadStream(path.join(process.cwd(), "/" + req.body.imgPath));
    // let len = 0;
    ...
    // readStream.on('end', () => {
    // });
    // let writeStream = fs.createWriteStream(path.join(process.cwd(), '../public/' + targetPath));
    ...
    //     if (err) {
    //         res.json({ status: 401, message: '提交失败', error: err });
    //     } else {
    //         res.json({ status: 200, message: "提交成功" })

    //         //  fs.renameSync(path.join(process.cwd(), "/" + req.body.imgPath), path.join(process.cwd(), '../public/' + targetPath))

    //     }
    // });


    // fs.unlinkSync('C:/Users/86138/Desktop/csdd-plugin/public/a.txt');
    // fs.renameSync(path.join(process.cwd(), "/" + req.body.imgPath), path.join(process.cwd(), '../public/' + targetPath))
    ...
    let promise_1 = new Promise((resolve, reject) => {
        let targetPath = 'imgs/' + req.query.title + '.' + req.query.imgPath.split('\\')[1].split('.')[1]
         fs.renameSync(path.join(process.cwd(), "/" + req.query.imgPath), path.join(process.cwd(), '../public/' + targetPath), function (err) {
            if (err) {
                // return res.sendResult(null, 400, "上传图片失败");
                reject('0')
            } else {
    ...
    let promise_2 = new Promise((resolve, reject) => {
        models.Plugin(req.query).save((err, data) => {
            console.log('err --', err, 'data --', data);
            if (err) {
                // res.json({ status: 401, message: '提交失败', error: err });
                reject('11')
    ...
    Promise.all([promise_1, promise_2]).then((datas) => {
        console.log('submit req ----', req.query, 'submit res -----', { res_1: datas[0], res_2: datas[1] });
        res.json({ status: 200, message: "提交成功" })
