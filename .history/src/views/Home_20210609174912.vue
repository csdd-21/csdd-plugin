<template>
  <div class="home">
    ...
    <div class="contentWrapper">
      ...
      <div class="content col-xs-12 col-md-6" v-for="(i, index) of life.datas" :key="index" @click="clickArticle(i)">
        <img  :src="i.imgUrl" :alt="i.title" />
        <div class="info " >
          <b>{{ i.title }}</b>
          <Tags :tags="[i.tag]"></Tags>
          <span class="desc"> {{ i.abstracts }}</span>
          <!-- <span class='url'>Go To Websites</span> -->
          <el-button><a :href="i.url">Websites</a></el-button>
        </div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...mapMutations(["SET_PARAMS_DEFAULT", "SET_PARAMS"]),
