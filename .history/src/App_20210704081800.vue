<template>
  <div id="app">
    ...
    <el-dialog title="提示" v-model="show.dialog" width="30%" :before-close="this.clear_form()">
      <div class="btns">
        <el-button @click="show.admin = true" v-if="!show.admin">管理员</el-button>
        <el-button @click="click_visitor" v-if="!show.admin">访客</el-button>
      </div>
      <el-form v-if="show.admin" :model="form" :rules="rules" ref="form" label-width="65px">
      ...
      <i class="hint" v-if="!show.admin"><i class="iconfont icon-xinghao" v-if="!show.admin"></i> 管理员可以直接发布文章，访客新增的文章需要在审核完后才会发布哦~</i>
    </el-dialog>
  </div>
</template>
