<template>
  <div class="new">
    <el-form :model="form" ref="refForm" :rules="rules" class="demo-form-inline" @submit.prevent>
  ...
  methods: {
    ...
    click_submit() {
      this.$refs["refForm"].validate(async (valid) => {
        if (valid) {
          await this.axios.post("/api/submit", this.form).then((res) => {
              ...
              if (res.data.status == 200) {
                this.$alert("文章会在管理员审核后24小时内发布", "", {
                    ...
                    console.log('this.form.auth',this.form.auth);
                    
                switch (this.form.auth) {
                  case "admin":
                    console.log('adminmmmmmmm');
                    this.$message({ message: "发布成功", type: "info" });
                    // this.$router.push({ name: "home" });
                    break;
                  case "visitor":
                    console.log('visitorrrrrrrrrrrr');
                    this.$alert("文章会在管理员审核后24小时内发布", "", {
                      confirmButtonText: "确定",
                      callback: (action) => {
                        this.$router.push({ name: "home" });
